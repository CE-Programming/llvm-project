...
bool Z80MachineLateOptimization::runOnMachineFunction(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    MachineInstr *AMirrorDef = nullptr;
    for (MachineBasicBlock::iterator I = MBB.begin(), E = MBB.end(); I != E;) {
      ...
      case TargetOpcode::COPY: {
        ...
        if (CopyDst == Z80::A && CopySrc == AMirroredInReg) {
          LLVM_DEBUG(dbgs() << "Erasing redundant copy to A from "
                     ...
                     MIB->dump());
          if (AMirrorDef) {
            assert(AMirrorDef->getOperand(0).getReg() == CopySrc);
            if (MIB->getOperand(1).isKill())
              AMirrorDef->getOperand(0).setIsDead();
            AMirrorDef->clearRegisterDeads(Z80::A);
          }
          MIB->eraseFromParent();
          Changed = true;
          continue;
