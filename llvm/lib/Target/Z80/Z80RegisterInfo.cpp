...
Z80RegisterInfo::getPointerRegClass(unsigned Kind) const {
  switch (Kind) {
...
const TargetRegisterClass *Z80RegisterInfo::getPointerRegClassForConstraint(
    const MachineFunction &, unsigned Constraint) const {
  unsigned Kind;
  switch (static_cast<InlineAsm::ConstraintCode>(Constraint)) {
  ...
  return getPointerRegClass(Kind);
}

const TargetRegisterClass *
Z80RegisterInfo::getLargestLegalSuperClass(const TargetRegisterClass *RC,
                                           const MachineFunction &) const {
  auto IsLegalSuperClass = [](const TargetRegisterClass *Super) {
    switch (Super->getID()) {
  ...
  if (IsLegalSuperClass(RC))
    return RC;
  for (unsigned ID : RC->superclasses())
    if (const TargetRegisterClass *Super = getRegClass(ID);
        IsLegalSuperClass(Super))
      return Super;
  return RC;
}
...
BitVector Z80RegisterInfo::getReservedRegs(const MachineFunction &MF) const {
  ...
  // IX must stay reserved even when no frame exists yet: register allocation
  // can create spill slots, which require IX as the frame pointer.
  for (Register Reg :
