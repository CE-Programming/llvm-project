...
bool Z80RegisterInfo::getRegAllocationHints(
  ...
  bool BaseResult = TargetRegisterInfo::getRegAllocationHints(
      VirtReg, Order, Hints, MF, VRM, Matrix);

  LLVM_DEBUG({
    if (!Hints.empty()) {
      dbgs() << "Z80RegHints: " << printReg(VirtReg, this) << " base hints: ";
      for (MCPhysReg H : Hints)
        dbgs() << printReg(H, this) << " ";
      dbgs() << "\n";
    }
  });
  ...
  // only apply hints to G24/G16 class where we have freedom to choose
  // between HL, DE, and BC. Other classes are already constrained
  const TargetRegisterClass *RC = MRI->getRegClass(VirtReg);
  const TargetRegisterClass *GRC =
      Is24Bit ? &Z80::G24RegClass : &Z80::G16RegClass;
  if (RC != GRC) {
    LLVM_DEBUG(dbgs() << "Z80RegHints: " << printReg(VirtReg, this)
                      << " skipped - RC=" << getRegClassName(RC)
                      << " (need " << getRegClassName(GRC) << ")\n");
    return BaseResult;
  }

  // how this vreg is used to determine if its
  // likely to be live during frame index rewriting operations
  SmallPtrSet<const MachineBasicBlock *, 4> DefBBs;
  SmallPtrSet<const MachineBasicBlock *, 8> UseBBs;
  SmallPtrSet<const MachineBasicBlock *, 8> ScannedBBs;
  SmallPtrSet<const MachineBasicBlock *, 8> BBsWithFI;

  bool IsPHIDefined = false;
  unsigned UseCount = 0;
  ...
  for (const MachineOperand &MO : MRI->reg_nodbg_operands(VirtReg)) {
    ...
    if (ScannedBBs.insert(MBB).second) {
      for (const MachineInstr &OtherMI : *MBB) {
        for (const MachineOperand &Op : OtherMI.operands()) {
          if (Op.isFI()) {
            BBsWithFI.insert(MBB);
            goto next_bb;
          }
        }
      }
  ...
  // check for cross-BB liveness. used in a BB where it wasnt defined
  // cross-BB vregs have extended live ranges likely to span FI operations
  bool CrossBBLive = false;
  for (const MachineBasicBlock *UseBB : UseBBs) {
    if (!DefBBs.count(UseBB)) {
      CrossBBLive = true;
      break;
    }
  }

  // check if any of the BBs where this vreg lives have FI operations
  bool HasFIInLiveBBs = false;
  for (const MachineBasicBlock *UseBB : UseBBs) {
    if (BBsWithFI.count(UseBB)) {
      HasFIInLiveBBs = true;
      break;
    }
  }
  if (!HasFIInLiveBBs) {
    for (const MachineBasicBlock *DefBB : DefBBs) {
      if (BBsWithFI.count(DefBB)) {
        HasFIInLiveBBs = true;
        break;
      }
    }
  ...
  // case B: cross-BB live vregs span multiple BBs, likely to be live
  // during FI rewriting operations scattered throughout the function
  if (CrossBBLive && HasFIInLiveBBs) {
    ShouldHintDE = true;
    HintReason = "CrossBB+FI";
  }

  // case C: moderate use count in FI rich BBs
  if (UseCount >= 2 && HasFIInLiveBBs) {
    ShouldHintDE = true;
    HintReason = "HighUse+FI";
  }

  LLVM_DEBUG({
    dbgs() << "Z80RegHints: " << printReg(VirtReg, this)
           << " analysis: PHI=" << IsPHIDefined
           << ", CrossBB=" << CrossBBLive
           << ", Uses=" << UseCount
           << ", HasFI=" << HasFIInLiveBBs
           << ", DefBBs=" << DefBBs.size()
           << ", UseBBs=" << UseBBs.size()
