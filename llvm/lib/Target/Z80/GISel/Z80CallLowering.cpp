...
struct Z80OutgoingValueHandler : public CallLowering::OutgoingValueHandler {
  ...
  void assignValueToAddress(Register ValVReg, Register Addr, LLT MemTy,
    ...
    auto MMO = MIRBuilder.getMF().getMachineMemOperand(
        MPO, MachineMemOperand::MOStore, VA.getLocVT().getStoreSize(), Align());
    MIRBuilder.buildStore(ValVReg, Addr, *MMO);
  }

  void finish(CallingConv::ID CC) {
    if (CC == CallingConv::Z80_TIFlags) {
...
struct CallArgHandler : public Z80OutgoingValueHandler {
  ...
  void assignValueToAddress(Register ValVReg, Register Addr, LLT MemTy,
    ...
    constrainOperandRegClass(MIRBuilder.getMF(), *STI.getRegisterInfo(),
                             ...
                             *STI.getRegBankInfo(), PushI, PushI.getDesc(),
                             PushI.getOperand(0), 0);
    ++StackPushes;
    MIRBuilder.setInsertPt(MIRBuilder.getMBB(), std::next(SaveInsertPt));
    SetupFrameAdjustment += SlotTy.getSizeInBytes();
  }

  void finish(CallingConv::ID CC, uint64_t StackSz) {
    FrameSize = StackSz;
    Z80OutgoingValueHandler::finish(CC);
    MIRBuilder.setInsertPt(MIRBuilder.getMBB(), RegCopies);
  }

  unsigned getPreFrameAdjustment() const { return 0; }
  ...
  unsigned getSetupFrameAdjustment() const { return SetupFrameAdjustment; }

  unsigned getDestroyFrameAdjustment() const { return 0; }
...
struct Z80IncomingValueHandler : public CallLowering::IncomingValueHandler {
  ...
  void assignValueToAddress(Register ValVReg, Register Addr, LLT MemTy,
    ...
    auto MMO = MIRBuilder.getMF().getMachineMemOperand(
        ...
        Align());
    LLT DstTy = MRI.getType(ValVReg);
    Register LoadReg = ValVReg;
    if (DstTy != MemTy)
      LoadReg = MRI.createGenericVirtualRegister(MemTy);
    MIRBuilder.buildLoad(LoadReg, Addr, *MMO);

    if (LoadReg == ValVReg)
      return;

    auto scalarizePtr = [&](Register Reg, LLT Ty) -> std::pair<Register, LLT> {
      if (!Ty.isPointer())
        return {Reg, Ty};
      LLT IntTy = LLT::scalar(Ty.getSizeInBits());
      Register IntReg = MIRBuilder.buildPtrToInt(IntTy, Reg).getReg(0);
      return {IntReg, IntTy};
    };

    if (DstTy.isPointer()) {
      LLT IntPtrTy = LLT::scalar(DstTy.getSizeInBits());
      auto [IntReg, SrcTy] = scalarizePtr(LoadReg, MemTy);
      if (TypeSize::isKnownLT(SrcTy.getSizeInBits(), IntPtrTy.getSizeInBits()))
        IntReg = MIRBuilder.buildAnyExt(IntPtrTy, IntReg).getReg(0);
      else if (TypeSize::isKnownGT(SrcTy.getSizeInBits(),
                                   IntPtrTy.getSizeInBits()))
        IntReg = MIRBuilder.buildTrunc(IntPtrTy, IntReg).getReg(0);
      MIRBuilder.buildIntToPtr(ValVReg, IntReg);
      return;
    }

    auto [CastSrcReg, CastSrcTy] = scalarizePtr(LoadReg, MemTy);
    if (TypeSize::isKnownGT(DstTy.getSizeInBits(), CastSrcTy.getSizeInBits())) {
      switch (VA.getLocInfo()) {
      case CCValAssign::SExt:
        MIRBuilder.buildSExt(ValVReg, CastSrcReg);
        break;
      case CCValAssign::ZExt:
        MIRBuilder.buildZExt(ValVReg, CastSrcReg);
        break;
      default:
        MIRBuilder.buildAnyExt(ValVReg, CastSrcReg);
        break;
      }
      return;
    }

    if (TypeSize::isKnownLT(DstTy.getSizeInBits(), CastSrcTy.getSizeInBits())) {
      MIRBuilder.buildTrunc(ValVReg, CastSrcReg);
      return;
    }

    MIRBuilder.buildCopy(ValVReg, CastSrcReg);
  }
  ...
  void assignValueToReg(Register ValVReg, Register PhysReg,
    ...
    LLT DstTy = MRI.getType(ValVReg);
    const auto &TRI =
        *MIRBuilder.getMF().getSubtarget<Z80Subtarget>().getRegisterInfo();
    TypeSize SrcSize = TRI.getRegSizeInBits(PhysReg, MRI);
    TypeSize DstSize = DstTy.getSizeInBits();
    ...
    assert(!SrcSize.isScalable() && "physical register size should be fixed");
    Register SrcReg =
        MRI.createGenericVirtualRegister(LLT::scalar(SrcSize.getFixedValue()));
    MIRBuilder.buildCopy(SrcReg, PhysReg);

    if (DstTy.isPointer()) {
      LLT IntPtrTy = LLT::scalar(DstTy.getSizeInBits());
      Register IntReg = SrcReg;
      if (TypeSize::isKnownLT(SrcSize, IntPtrTy.getSizeInBits()))
        IntReg = MIRBuilder.buildAnyExt(IntPtrTy, SrcReg).getReg(0);
      else if (TypeSize::isKnownGT(SrcSize, IntPtrTy.getSizeInBits()))
        IntReg = MIRBuilder.buildTrunc(IntPtrTy, SrcReg).getReg(0);
      MIRBuilder.buildIntToPtr(ValVReg, IntReg);
      return;
    }

    if (TypeSize::isKnownLT(SrcSize, DstSize)) {
      switch (VA.getLocInfo()) {
      case CCValAssign::SExt:
        MIRBuilder.buildSExt(ValVReg, SrcReg);
        break;
      case CCValAssign::ZExt:
        MIRBuilder.buildZExt(ValVReg, SrcReg);
        break;
      default:
        MIRBuilder.buildAnyExt(ValVReg, SrcReg);
        break;
      }
      return;
    }

    MIRBuilder.buildTrunc(ValVReg, SrcReg);
  }
...
bool Z80CallLowering::lowerTailCall(MachineIRBuilder &MIRBuilder,
  ...
  unsigned TCRetOpc = Info.Callee.isReg()
                      ...
                      : Is24Bit ? Z80::TCRETURN24
                                : Z80::TCRETURN16;
  auto MIB = MIRBuilder.buildInstrNoInsert(TCRetOpc)
                 .add(Info.Callee)
...
bool Z80CallLowering::lowerCall(MachineIRBuilder &MIRBuilder,
  ...
  return lowerCallImpl(MIRBuilder, Info, Z80::COND_INVALID);
}

bool Z80CallLowering::lowerConditionalCall(MachineIRBuilder &MIRBuilder,
                                           CallLoweringInfo &Info,
                                           unsigned Cond) const {
  assert(Cond <= Z80::LAST_VALID_COND && "Invalid condition");
  return lowerCallImpl(MIRBuilder, Info, Cond);
}

bool Z80CallLowering::lowerCallImpl(MachineIRBuilder &MIRBuilder,
                                    CallLoweringInfo &Info,
                                    unsigned Cond) const {
  MachineFunction &MF = MIRBuilder.getMF();
  const Function &F = MF.getFunction();
  ...
  bool IsConditional = Cond != Z80::COND_INVALID;
  bool CanTailCallOpt =
      !IsConditional &&
  ...
  bool Is24Bit = STI.is24Bit();
  assert((!IsConditional || !Info.Callee.isReg()) &&
         "Conditional indirect calls are unsupported");
  unsigned CallOpc = Info.Callee.isReg() ? Is24Bit ? Z80::CALL24r : Z80::CALL16r
                     : IsConditional ? Is24Bit ? Z80::CALL24CC : Z80::CALL16CC
                     : Is24Bit       ? Z80::CALL24
                                     : Z80::CALL16;

  auto MIB = MIRBuilder.buildInstrNoInsert(CallOpc).add(Info.Callee);
  if (IsConditional)
    MIB.addImm(Cond);
  ...
  if (!determineAndHandleAssignments(Handler, Assigner, OutArgs, MIRBuilder,
    ...
    return false;
  Handler.finish(Info.CallConv, Assigner.StackSize);
...
bool Z80CallLowering::lowerFormalArguments(MachineIRBuilder &MIRBuilder,
  ...
  FuncInfo.setArgFrameSize(Assigner.StackSize);
  if (F.isVarArg()) {
    int FrameIdx =
        MF.getFrameInfo().CreateFixedObject(1, Assigner.StackSize, true);
...
bool Z80CallLowering::lowerReturn(MachineIRBuilder &MIRBuilder,
  ...
  if (!VRegs.empty()) {
    ...
    if (!determineAndHandleAssignments(Handler, Assigner, SplitArgs, MIRBuilder,
                                       F.getCallingConv(), F.isVarArg()))
      return false;
    Handler.finish(F.getCallingConv());
  }
