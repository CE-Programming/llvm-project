...
bool Z80InstructionSelector::select(MachineInstr &I) const {
  ...
  // Keep this zext form on the C++ selection path. In v19, the imported
  // matcher path can produce a dead-flag variant that in turn drives a
  // larger spill/reload sequence in some loops.
  if (I.getOpcode() == TargetOpcode::G_ZEXT) {
    ...
    if (SrcTy.isScalar() && DstTy.isScalar()) {
      ...
      if ((SrcBits == 1 && (DstBits == 8 || DstBits == 16 || DstBits == 24)) ||
          (SrcBits == 8 && (DstBits == 16 || DstBits == 24)) ||
          (SrcBits == 16 && DstBits == 24) ||
          (STI.is24Bit() && SrcBits == 32 && DstBits == 64))
        return selectZExt(I, MRI);
    }
  }

  if (selectImpl(I, *CoverageInfo)) {
    // Preserve the v18 liveness shape for TableGen-selected LD24r0. In v19,
    // marking the implicit F def dead here can drive excessive IY
    // rematerialization in some loops.
    if (I.getOpcode() == Z80::LD24r0) {
