...
static int64_t getSubRegIndex(unsigned Width, unsigned Off = 0) {
  ...
  case 16:
    ...
    case 16:
      return Z80::sub_word1;
    case 32:
      return Z80::sub_word2;
    case 48:
      return Z80::sub_word3;
    }
    break;
  case 24:
    switch (Off) {
    case 0:
      return Z80::sub_low24;
    case 24:
      return Z80::sub_mid24;
    }
    break;
  }
...
bool Z80InstructionSelector::selectTrap(MachineInstr &I) const {
  ...
  if (!isValidTrapRstVector(Z80TrapRstVector))
    report_fatal_error("invalid -z80-trap-rst-vector value, expected one of "
...
bool Z80InstructionSelector::select(MachineInstr &I) const {
  ...
  // The imported add patterns constrain operands too early and regress
  // allocation, while the subtract patterns using physical HL/UHL cant be
  // imported
  ...
  // Imported zext patterns make the LD8r0/LD24r0 flag def dead, which can
  // increase register pressure and spill code
  if (I.getOpcode() == TargetOpcode::G_ZEXT) {
    ...
    if (SrcTy.isScalar() && DstTy.isScalar()) {
      ...
      if ((SrcBits == 8 && (DstBits == 16 || DstBits == 24)) ||
          (SrcBits == 16 && DstBits == 24))
...
bool Z80InstructionSelector::selectConstant(MachineInstr &I,
  ...
  if (I.getOpcode() == TargetOpcode::G_CONSTANT && Ty.getSizeInBits() == 24) {
    const int64_t Value = I.getOperand(1).getCImm()->getSExtValue();
    if (Value == 0 || Value == -1) {
      MachineIRBuilder MIB(I);
      unsigned Opc = Value == 0 ? Z80::LD24r0 : Z80::LD24r_1;
      auto NewMI = MIB.buildInstr(Opc, {DefReg}, {});
...
bool Z80InstructionSelector::selectSExt(MachineInstr &I,
  ...
  if (SrcTy == LLT::scalar(1)) {
    ...
    case 32: {
      ...
      if (Is24Bit) {
        ...
        Fill24I->findRegisterUseOperand(Z80::UHL, nullptr)->setIsUndef();
        ...
        Fill16I->findRegisterUseOperand(Z80::HL, nullptr)->setIsUndef();
    ...
    Fill->findRegisterUseOperand(FillReg, nullptr)->setIsUndef();
  ...
  if (DstSize > 24) {
    ...
    auto ExtractByteFrom24 = [&](Register Src24,
      ...
      Register ByteReg = createGenericVirtualRegister(MRI, LLT::scalar(8));
      int FI = MF.getFrameInfo().CreateStackObject(3, Align(1), false);
      auto Store =
          MIB.buildInstr(Z80::LD24or).addFrameIndex(FI).addImm(0).addReg(Src24);
    ...
    auto CopySubReg = [&](LLT Ty, Register Reg, unsigned SubIdx) -> Register {
      ...
      return constrainSelectedInstRegOperands(*Copy, TII, TRI, RBI)
                 ? Tmp
                 : Register();
    };
    ...
    auto BuildFill8FromSignByte = [&](Register SignByte) -> Register {
      ...
      Fill8I->findRegisterUseOperand(Z80::A, nullptr)->setIsUndef();
    ...
    auto GetSignByte = [&]() -> Register {
      ...
      case 32:
        ...
        if (Register W1 = CopySubReg(LLT::scalar(16), SrcReg, Z80::sub_word1))
    ...
    if (!Is24Bit) {
      if (DstSize == 32) {
        if (SrcSize != 8 && SrcSize != 16)
          return false;
        Register Lo16 =
            SrcSize == 16 ? SrcReg : BuildSelectedSExt(LLT::scalar(16), SrcReg);
        if (!Lo16)
          return false;
        ...
        auto SeqI = MIB.buildInstr(
            ...
            {Lo16, int64_t(Z80::sub_short), Fill16, int64_t(Z80::sub_word1)});
        if (!constrainSelectedInstRegOperands(*SeqI, TII, TRI, RBI))
          return false;
        I.eraseFromParent();
        return true;
    ...
    if (DstSize == 32) {
      if (SrcSize != 8 && SrcSize != 16 && SrcSize != 24)
        return false;
      Register Low24 =
          SrcSize == 24 ? SrcReg : BuildSelectedSExt(LLT::scalar(24), SrcReg);
      if (!Low24)
        return false;
      Register SignByte = GetSignByte();
      if (!SignByte)
      ...
      auto SeqI = MIB.buildInstr(
          TargetOpcode::REG_SEQUENCE, {DstReg},
          {Low24, int64_t(Z80::sub_low24), Fill8, int64_t(Z80::sub_high8)});
      if (!constrainSelectedInstRegOperands(*SeqI, TII, TRI, RBI))
        return false;
      I.eraseFromParent();
      return true;
    ...
    if (DstSize == 48) {
      ...
      Register High24;
      if (SrcSize == 32) {
        Low24 = CopySubReg(LLT::scalar(24), SrcReg, Z80::sub_low24);
        Register Hi8 = CopySubReg(LLT::scalar(8), SrcReg, Z80::sub_high8);
        if (!Low24 || !Hi8)
          return false;
      ...
      auto SeqI = MIB.buildInstr(
          TargetOpcode::REG_SEQUENCE, {DstReg},
          {Low24, int64_t(Z80::sub_low24), High24, int64_t(Z80::sub_mid24)});
      if (!constrainSelectedInstRegOperands(*SeqI, TII, TRI, RBI))
        return false;
      I.eraseFromParent();
      return true;
    ...
    if (DstSize == 64) {
      ...
      if (SrcSize == 48) {
        Low24 = CopySubReg(LLT::scalar(24), SrcReg, Z80::sub_low24);
        Mid24 = CopySubReg(LLT::scalar(24), SrcReg, Z80::sub_mid24);
        if (!Low24 || !Mid24)
          return false;
        ...
        auto StoreLow = MIB.buildInstr(Z80::LD24or)
                            .addFrameIndex(FI)
                            .addImm(0)
                            .addReg(SrcReg, 0, Z80::sub_low24);
        auto StoreHi = MIB.buildInstr(Z80::LD8or)
                           .addFrameIndex(FI)
                           .addImm(3)
  ...
  if (SrcSize == 16) {
    ...
    if (!RBI.constrainGenericRegister(CopyHigh.getReg(0), Z80::R8RegClass, MRI))
      return false;
...
bool Z80InstructionSelector::selectZExt(MachineInstr &I,
  ...
  if (SrcTy == LLT::scalar(1)) {
    ...
    if (!RBI.constrainGenericRegister(CopyFromA.getReg(0), Z80::R8RegClass,
                                      MRI))
      return false;
  ...
  //   (i16 (zext R8:$src)) -> (REG_SEQUENCE R16, (LD8r0), sub_high, R8:$src,
  //   sub_low) (i24 (zext R8:$src)) -> (INSERT_SUBREG (LD24r0), R8:$src,
  //   sub_low) (i24 (zext R16:$src)) -> (INSERT_SUBREG (LD24r0), R16:$src,
  //   sub_short)
  ...
  MachineIRBuilder MIB(I);
  auto PreserveFDefLiveness = [](MachineInstrBuilder MI) {
  ...
  if (Is24Bit && SrcSize == 32 && DstSize == 64) {
    ...
    Register Hi8Reg = createGenericVirtualRegister(MRI, LLT::scalar(8));
    auto LoadLow =
        MIB.buildInstr(Z80::LD24ro, {Lo24Reg}, {}).addFrameIndex(FI).addImm(0);
    auto LoadHi =
        MIB.buildInstr(Z80::LD8ro, {Hi8Reg}, {}).addFrameIndex(FI).addImm(3);
    if (!constrainSelectedInstRegOperands(*LoadLow, TII, TRI, RBI) ||
        !constrainSelectedInstRegOperands(*LoadHi, TII, TRI, RBI))
      return false;
    if (!RBI.constrainGenericRegister(Lo24Reg, Z80::R24RegClass, MRI) ||
    ...
    if (!constrainSelectedInstRegOperands(*Mid24Zero, TII, TRI, RBI))
      return false;
    PreserveFDefLiveness(Mid24Zero);
    ...
    auto High16Zero =
        MIB.buildInstr(Z80::LD16ri, {&Z80::R16RegClass}, {int64_t(0)});
  ...
  if (SrcSize == 8 && DstSize == 16) {
    ...
    if (!constrainSelectedInstRegOperands(*ZeroHigh, TII, TRI, RBI))
      return false;
    // Keep F live here to preserve the v18 selection/regalloc behavior for
    // this zext sequence. In v19, marking the implicit F def dead can steer
    // allocation toward an extra spill/reload pattern in some loops.
    for (MachineOperand &MO : ZeroHigh->operands()) {
      if (MO.isReg() && MO.isDef() && MO.getReg() == Z80::F)
        MO.setIsDead(false);
    }
    MIB.buildInstr(TargetOpcode::REG_SEQUENCE, {DstReg},
                   {ZeroHigh.getReg(0), uint64_t(Z80::sub_high), SrcReg,
                    uint64_t(Z80::sub_low)});
  } else if (SrcSize == 8 && DstSize == 24) {
    ...
    LLVM_DEBUG(dbgs() << "Unsupported G_ZEXT: s" << SrcSize << " -> s"
                      << DstSize << "\n");
    return false;
  }
...
bool Z80InstructionSelector::selectFrameIndexOrGep(MachineInstr &I,
  ...
  if (I.getOpcode() == TargetOpcode::G_PTR_ADD) {
    ...
    if (OffConst && OffConst->sge(-1) && OffConst->sle(1)) {
      ...
      case 8:
        Opc = IsInc ? Z80::INC8r : Z80::DEC8r;
        break;
      case 16:
        Opc = IsInc ? Z80::INC16r : Z80::DEC16r;
        break;
      case 24:
        Opc = IsInc ? Z80::INC24r : Z80::DEC24r;
        break;
      }
      I.setDesc(TII.get(Opc));
      return constrainSelectedInstRegOperands(I, TII, TRI, RBI);
...
bool Z80InstructionSelector::selectExtract(MachineInstr &I,
  ...
  auto NormalizeRC = [&](Register Reg, const TargetRegisterClass *&RC) {
    if (RC == &Z80::R8RegClass || RC == &Z80::R16RegClass ||
        RC == &Z80::R24RegClass)
      return true;
    const TargetRegisterClass *NewRC = selectRRegClass(Reg, MRI);
    if (!NewRC)
      return false;
    if (!Reg.isPhysical() && !RBI.constrainGenericRegister(Reg, *NewRC, MRI))
      return false;
    RC = NewRC;
    return true;
  };
  if (!NormalizeRC(DstReg, DstRC) || !NormalizeRC(SrcReg, SrcRC))
    return false;

  unsigned Offset = I.getOperand(2).getImm();

  // In 24-bit mode, R32_24 values are modeled as (low24, high8). Extracting a
  // 16-bit slice at bit 16 is not representable as a single subregister, so
  // spill/reload through memory instead of emitting EXTRACT_SUBREG.
  if (STI.is24Bit() && MRI.getType(SrcReg) == LLT::scalar(32) &&
      MRI.getType(DstReg) == LLT::scalar(16) && Offset == 16) {
    MachineIRBuilder MIB(I);
    int FI = MF.getFrameInfo().CreateStackObject(4, Align(1), false);
    Register Low24 = createGenericVirtualRegister(MRI, LLT::scalar(24));
    Register High8 = createGenericVirtualRegister(MRI, LLT::scalar(8));
    MIB.buildInstr(TargetOpcode::COPY, {Low24}, {})
        .addReg(SrcReg, 0, Z80::sub_low24);
    MIB.buildInstr(TargetOpcode::COPY, {High8}, {})
        .addReg(SrcReg, 0, Z80::sub_high8);
    if (!RBI.constrainGenericRegister(Low24, Z80::R24RegClass, MRI) ||
        !RBI.constrainGenericRegister(High8, Z80::R8RegClass, MRI))
      return false;
    MIB.buildInstr(Z80::LD24or).addFrameIndex(FI).addImm(0).addReg(Low24);
    MIB.buildInstr(Z80::LD8or).addFrameIndex(FI).addImm(3).addReg(High8);
    unsigned LoadOpc = STI.has16BitEZ80Ops() ? Z80::LD16ro : Z80::LD88ro;
    MIB.buildInstr(LoadOpc, {DstReg}, {}).addFrameIndex(FI).addImm(2);
    I.eraseFromParent();
  ...
  if (SubIdx != Z80::NoSubRegister &&
    ...
    auto BuildStore = [&](unsigned StoreOpc, Register Reg, int FI,
      ...
      auto Store =
          MIB.buildInstr(StoreOpc).addFrameIndex(FI).addImm(ByteOffset);
    ...
    if (STI.is24Bit() && SrcRC == &Z80::R16RegClass) {
      ...
      if (SrcReg.isPhysical())
        WideSrcReg = TRI.getMatchingSuperReg(SrcReg, Z80::sub_short, WideSrcRC);
      else
    ...
    if (STI.is24Bit() && DstRC == &Z80::R16RegClass) {
      ...
      if (DstReg.isPhysical())
        WideDstReg = TRI.getMatchingSuperReg(DstReg, Z80::sub_short, WideDstRC);
      else
...
bool Z80InstructionSelector::selectMergeValues(MachineInstr &I,
  ...
  case 24: {
    ...
    if (mi_match(UpReg, MRI, m_GAShr(m_Reg(TmpReg), m_SpecificICst(7)))) {
      ...
      SbcI->findRegisterUseOperand(Z80::UHL, nullptr)->setIsUndef();
      ...
      auto Store = MIB.buildInstr(Z80::LD8or)
                       .addFrameIndex(FI)
                       .addImm(0)
                       .add(I.getOperand(3));
...
Z80InstructionSelector::foldCompare(MachineInstr &I, MachineIRBuilder &MIB,
  ...
  if (OpSize == 8 && IsSigned && ConstRHS && !ConstRHS->Value &&
      (CC == Z80::COND_P || CC == Z80::COND_M)) {
    auto Bit = MIB.buildInstr(Z80::BIT8gb, {}, {LHSReg, uint64_t(7)});
    if (!constrainSelectedInstRegOperands(*Bit, TII, TRI, RBI))
      return Z80::COND_INVALID;
    return CC == Z80::COND_M ? Z80::COND_NZ : Z80::COND_Z;
  }
  ...
  if (NeedSignedOverflowFix) {
    ...
    KnownBits LHSKnown = this->VT->getKnownBits(LHSReg);
    ...
    if (ConstRHS) {
      ...
      KnownBits RHSKnown = this->VT->getKnownBits(RHSReg);
      RHSMin = RHSKnown.getSignedMinValue().getSExtValue();
      RHSMax = RHSKnown.getSignedMaxValue().getSExtValue();
    }

    NeedSignedOverflowFix =
        LHSMin - RHSMax < SignedMin || LHSMax - RHSMin > SignedMax;
  }
  ...
  if (ConstRHS) {
    if (!ConstRHS->Value &&
        (CC == Z80::COND_Z || CC == Z80::COND_NZ ||
         (IsSigned && (CC == Z80::COND_P || CC == Z80::COND_M)))) {
      if (OpSize == 8) {
        Register SrcReg;
        int64_t Mask;
        if (mi_match(LHSReg, MRI,
          ...
          Reg = {};
          Ops = {SrcReg,
                 uint64_t(llvm::countr_zero(static_cast<uint64_t>(Mask)))};
        } else {
          Opc = Z80::OR8ar;
          Ops = {Reg};
        }
        ...
        // The expansion handles copying to HL/UHL internally, so skip the
        // external COPY.
        Opc = OpSize == 24 ? Z80::Cmp24a0 : Z80::Cmp16a0;
        Ops = {LHSReg};
        Reg = {}; // Clear Reg to skip COPY below - copy is done internally by
                  // expansion
      }
    } else if (OpSize == 8) {
      Opc = Z80::CP8ai;
...
Z80::CondCode Z80InstructionSelector::foldExtendedAddSub(
  ...
  case 8:
    ...
    break;
  case 16:
    AddSubOpc = IsAdd ? (IsExtend || IsSigned) ? Z80::ADC16ao : Z80::ADD16ao
                      : Z80::SBC16ao;
    ...
    break;
  case 24:
    AddSubOpc = IsAdd ? (IsExtend || IsSigned) ? Z80::ADC24ao : Z80::ADD24ao
                      : Z80::SBC24ao;
...
Z80InstructionSelector::foldSetCC(MachineInstr &I, MachineIRBuilder &MIB,
  ...
  for (MachineBasicBlock::iterator II = I, EI = MIB.getInsertPt(); II != EI;
       ++II) {
    if (II == MBB.end())
      return Z80::COND_INVALID;
...
bool Z80InstructionSelector::selectShift(MachineInstr &I,
  ...
  if ((Opc == TargetOpcode::G_LSHR || Opc == TargetOpcode::G_ASHR) &&
    ...
    MIB.buildInstr(TargetOpcode::REG_SEQUENCE, {DstReg},
                   {LoOut.getReg(0), uint64_t(Z80::sub_low), HiOut.getReg(0),
                    uint64_t(Z80::sub_high)});
    I.eraseFromParent();
    return RBI.constrainGenericRegister(DstReg, *WordRC, MRI);
  }

  // Handle s16/s24 G_SHL by 2-6 with inline ADD instructions.
  // For s24, use ADD24aa for all adds (generates add hl, hl, 1 byte each).
  // For s16 in 24-bit mode, use SUBREG_TO_REG to promote to s24, then
  // (N-1) * ADD24aa (1 byte each) + final ADD16aa (2 bytes) for truncation.
  // This saves 1 byte per intermediate add versus all ADD16aa.
  // For s16 in 16-bit mode, use ADD16aa.
  if (Opc == TargetOpcode::G_SHL && Amt->Value.uge(2) && Amt->Value.ule(6) &&
    ...
    // In 24-bit mode, promote s16 to s24. The undefined upper byte and
    // unobservable carry let each intermediate shift use one-byte ADD24aa.
    if (TySize == 16 && STI.is24Bit()) {
      ...
      if (!DstRC &&
          !RBI.constrainGenericRegister(DstReg, Z80::R16RegClass, MRI)) {
        return false;
      }
      I.eraseFromParent();
      return true;
    ...
    // Standard path: s24, or s16 in 16-bit mode.
    unsigned AddOpc;
    Register AccumReg;
    ...
    // copy source to accumulator - source can be any register, we are copying
    // TO the accumulator
    ...
    for (unsigned i = 0; i < ShiftAmt; ++i) {
      BuildMI(MBB, InsertPt, DL, TII.get(AddOpc), AccumReg).addReg(AccumReg);
    }
...
// V15-style instruction selection for G_ADD/G_SUB.
//
...
// 3. This avoids the register pressure issues caused by deferred pseudo
// expansion
//
...
bool Z80InstructionSelector::selectAddSub(MachineInstr &I,
  ...
  const TargetRegisterClass *AccumRC =
      Is24Bit ? &Z80::A24RegClass : &Z80::A16RegClass;
  const TargetRegisterClass *OperandRC =
  ...
  if (IsSub) {
    ...
    auto SubI = MIB.buildInstr(Is24Bit ? Z80::Sub24ao : Z80::Sub16ao)
                    .addReg(ActualSrc2);
...
bool Z80InstructionSelector::selectSetCond(unsigned ExtOpc, Register DstReg,
  ...
  if (CC != DirectCC) {
    ...
    if (FillOpc == Z80::SBC8ar) {
      ...
      CopyUndefI->findRegisterUseOperand(FillReg, nullptr)->setIsUndef();
      UndefReg = CopyUndefI.getReg(0);
      if (!RBI.constrainGenericRegister(UndefReg, *DstRC, MRI))
        return false;
    }
    auto FillI = MIB.buildInstr(FillOpc);
    FillI->findRegisterUseOperand(FillReg, nullptr)->setIsUndef();
    if (UndefReg)
...
bool Z80InstructionSelector::selectBrJT(MachineInstr &I,
  ...
  auto Load = MIB.buildInstr(STI.is24Bit()      ? Z80::LD24rp
                             : STI.hasEZ80Ops() ? Z80::LD16rp
                                                : Z80::LD88rp,
                             {AddrTy}, {AddrReg});
...
bool Z80InstructionSelector::selectInlineAsm(MachineInstr &I,
  ...
  for (unsigned Idx :
    ...
    if (const auto *RC = I.getRegClassConstraint(Idx, &TII, &TRI)) {
      (void)RC;
      Reg = constrainOperandRegClass(*MF, TRI, MRI, TII, RBI, I, I.getDesc(),
                                     MO, Idx);
    }
  }
  return true;
