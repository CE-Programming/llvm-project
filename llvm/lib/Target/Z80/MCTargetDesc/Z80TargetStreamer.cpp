...
void Z80TargetAsmStreamer::emitCode16() {
  OS << (Z80GasStyle ? "\t.assume\tADL = 0\n" : "\tassume\tadl = 0\n");
}

void Z80TargetAsmStreamer::emitCode24() {
  OS << (Z80GasStyle ? "\t.assume\tADL = 1\n" : "\tassume\tadl = 1\n");
}
...
void Z80TargetAsmStreamer::emitAlign(Align Alignment) {
  ...
  } else if (auto Mask = Alignment.value() - 1) {
    OS << "\trb\t($$ - $) and " << Mask << '\n';
  }
}
