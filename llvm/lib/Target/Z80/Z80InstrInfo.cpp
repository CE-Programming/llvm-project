...
namespace llvm::Z80 {
struct Z80FrameAddrMappingInfo {
  unsigned OffsetOpcode;
  unsigned PointerOpcode;
};

#define GET_Z80FrameAddrMappingTable_DECL
#define GET_Z80FrameAddrMappingTable_IMPL
#include "Z80GenSearchableTables.inc"
} // namespace llvm::Z80
...
Z80InstrInfo::Z80InstrInfo(Z80Subtarget &STI)
    : Z80GenInstrInfo(STI, RI,
                      (STI.is24Bit() ? Z80::ADJCALLSTACKDOWN24
...
unsigned Z80InstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
  ...
  for (unsigned OpIdx = 0; OpIdx != 2; ++OpIdx) {
    ...
    EDPre = false;
    // (ix) gains an offset
    if (MI.getDesc().operands()[OpIdx].OperandType == MCOI::OPERAND_MEMORY)
      HasOff = true;
  }
...
void Z80InstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (DstReg == SrcReg)
    return;
  // Copying from a super-register to one of its sub-registers is a no-op at
  // this stage (e.g. UBC -> BC); the sub-register already aliases the value.
  if (RI.isSubRegisterEq(SrcReg, DstReg))
    return;
  ...
  if (Z80::R8RegClass.contains(DstReg, SrcReg)) {
    ...
    if (Z80::G8RegClass.contains(DstReg, SrcReg)) {
      ...
      bool NeedEX = false;
      for (Register *Reg : {&DstReg, &SrcReg}) {
        switch (*Reg) {
      ...
      if (NeedEX) {
        ...
        if (MI == MBB.begin() || std::prev(MI)->getOpcode() != ExOpc) {
          ...
          Ex->findRegisterUseOperand(Is24Bit ? Z80::UDE : Z80::DE, nullptr)
              ->setIsUndef();
          Ex->findRegisterUseOperand(Is24Bit ? Z80::UHL : Z80::HL, nullptr)
              ->setIsUndef();
        } else
      ...
      if (NeedEX)
        BuildMI(MBB, MI, DL, get(ExOpc))
          ->findRegisterUseOperand(Is24Bit ? Z80::UHL : Z80::HL, nullptr)->setIsUndef();
    }
    return;
  }
  ...
  if (Is24Bit == Subtarget.is24Bit()) {
    ...
    if (KillSrc && canExchange(DstReg, SrcReg)) {
      unsigned ExOpc = Is24Bit ? Z80::EX24DE : Z80::EX16DE;
      // If the prev instr was also EX DE,HL, they cancel out
      // Delete the previous one and skip emitting this one
      if (MI != MBB.begin() && std::prev(MI)->getOpcode() == ExOpc) {
        std::prev(MI)->eraseFromParent();
        return;
      }
      MachineInstrBuilder MIB = BuildMI(MBB, MI, DL, get(ExOpc));
      MIB->findRegisterUseOperand(SrcReg, nullptr)->setIsKill();
      MIB->findRegisterDefOperand(SrcReg, /*TRI=*/nullptr, /*isDead=*/false,
                                  /*Overlap=*/false)
          ->setIsDead();
      MIB->findRegisterUseOperand(DstReg, nullptr)->setIsUndef();
      return;
    }
    bool IsSrcIndexReg = Z80::I16RegClass.contains(SrcReg) ||
                         Z80::I24RegClass.contains(SrcReg);

    LLVM_DEBUG(dbgs() << "copyPhysReg: Is24Bit=" << Is24Bit
                      << " hasEZ80Ops=" << Subtarget.hasEZ80Ops()
                      << " IsSrcIndexReg=" << IsSrcIndexReg
                      << " Src=" << RI.getName(SrcReg) << "\n");

    // Special case copies from index registers when we have eZ80 ops.
    if (Subtarget.hasEZ80Ops() && IsSrcIndexReg) {
      LLVM_DEBUG(dbgs() << "  Emitting LEA copy\n");
  ...
  // Allow narrowing copies by selecting the low sub-register first.
  // This is needed for patterns like A <- UHL after regalloc.
  if (Z80::R8RegClass.contains(DstReg) &&
      (Z80::R16RegClass.contains(SrcReg) || Z80::R24RegClass.contains(SrcReg))) {
    copyPhysReg(MBB, MI, DL, DstReg, RI.getSubReg(SrcReg, Z80::sub_low),
                KillSrc);
    return;
  }
  if (Z80::R16RegClass.contains(DstReg) && Z80::R24RegClass.contains(SrcReg)) {
    copyPhysReg(MBB, MI, DL, DstReg, RI.getSubReg(SrcReg, Z80::sub_short),
                KillSrc);
    return;
  }
...
void Z80InstrInfo::storeRegToStackSlot(
    ...
    bool IsKill, int FI, const TargetRegisterClass *TRC, Register VReg,
  ...
  case Z80::F: {
    ...
    BuildMI(MBB, MI, DL, get(Is24Bit ? Z80::EX24sa : Z80::EX16sa), TempReg)
        .addReg(TempReg, RegState::Undef);
    storeRegToStackSlot(MBB, MI, Z80::L, true, FI, &Z80::R8RegClass, VReg);
    applySPAdjust(*BuildMI(MBB, MI, DL,
                           get(Is24Bit ? Z80::POP24r : Z80::POP16r), TempReg));
    return;
  }
  ...
  if (Is24Bit && Opc == Z80::LD24or && SrcReg.isPhysical())
    if (auto SuperReg = RI.getMatchingSuperReg(
...
void Z80InstrInfo::loadRegFromStackSlot(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI, Register DstReg,
    int FI, const TargetRegisterClass *TRC, Register VReg, unsigned SubReg,
    MachineInstr::MIFlag Flags) const {
  const DebugLoc &DL = MBB.findDebugLoc(MI);
  bool Is24Bit = Subtarget.is24Bit();
  ...
  case Z80::F: {
    ...
    applySPAdjust(
        *BuildMI(MBB, MI, DL, get(Is24Bit ? Z80::PUSH24r : Z80::PUSH16r))
             .addReg(TempReg, RegState::Undef));
    loadRegFromStackSlot(MBB, MI, Z80::L, FI, &Z80::R8RegClass, VReg);
    BuildMI(MBB, MI, DL, get(TargetOpcode::COPY), Z80::H)
        .addReg(Z80::A, RegState::Undef); // Preserve A
    BuildMI(MBB, MI, DL, get(Is24Bit ? Z80::EX24sa : Z80::EX16sa), TempReg)
        .addReg(TempReg, RegState::Undef);
  ...
  case 3:
    ...
    if (DstReg.isPhysical())
      if (auto SuperReg = RI.getMatchingSuperReg(DstReg, Z80::sub_short,
                                                &Z80::R24RegClass))
        DstReg = SuperReg;
    break;
  case 4: {
...
bool Z80InstrInfo::rewriteFrameIndex(MachineInstr &MI, unsigned FIOperandNum,
  ...
  bool IsSplitFrameAccess =
      MI.getDesc().TSFlags & Z80II::IsSplitFrameAccess;
  ...
  auto chooseFinalDisp = [&](int64_t TotalOff, int64_t &FinalDisp) {
    FinalDisp = 0;
    if (Opc == Z80::PEA24o || Opc == Z80::PEA16o)
      return false;

    unsigned FullChunks = countLEAChunks(TotalOff);
    if (FullChunks == 0)
      return false;
    ...
    unsigned BestChunks = countLEAChunks(TotalOff - BestDisp);
    if (BestChunks >= FullChunks)
      return false;
  ...
  unsigned ManualSpillAddCost = 2 + 1 + (Is24Bit ? 3 : 2) + 1;
  if (SaveFlags)
    ManualSpillAddCost += 2;
  ...
  auto selectSpillableNoOverlap = [&](const TargetRegisterClass *RC,
                                      Register Exclude0,
                                      Register Exclude1) -> Register {
    for (MCPhysReg Reg : *RC) {
      if (Reg == Exclude0 || Reg == Exclude1)
  ...
  auto emitOffsetAdd = [&](Register DstReg, Register OffsetReg, int64_t Adj,
    ...
    if (SpillOffsetReg) {
      assert(OffsetReg.isPhysical());
      assert(!regOverlapsMI(OffsetReg) &&
             "manual spill temp must not overlap rewritten MI");
      pushPhys(OffsetReg);
    }
    ...
    if (SaveFlags)
      applySPAdjust(
          *BuildMI(MBB, II, DL, get(Is24Bit ? Z80::PUSH24AF : Z80::PUSH16AF)))
          .findRegisterUseOperand(Z80::AF, nullptr)
          ->setIsUndef();
    BuildMI(MBB, II, DL, get(Is24Bit ? Z80::ADD24ao : Z80::ADD16ao), DstReg)
        .addReg(DstReg)
  ...
  if ((Opc == Z80::LEA24ro &&
    ...
    auto [OffsetReg, SpillOffsetReg] = selectOffsetTemp(/*Exclude0=*/Op0Reg,
                                                        /*Exclude1=*/BaseReg);
    if (SpillOffsetReg && !miDefinesPhys(OffsetReg))
      pushPhys(OffsetReg);
    copyRegister(MBB, II, DL, Op0Reg, BaseReg);
    emitOffsetAdd(Op0Reg, OffsetReg, NewOffset);
    if (SpillOffsetReg && !miDefinesPhys(OffsetReg))
      popPhysBeforeMI(OffsetReg);
    MI.eraseFromParent();
    return true;
  ...
  // chain is large enough that a controlled manual spill plus LDri+ADD is
  // better. when no scratch reg exists, using BaseReg (push/adjust/pop)
  // should need fewer temporaries and avoid cascading push/pop sequences inserted
  // by scavengeFrameVirtualRegs
  //
  ...
  if (ScratchReg) {
    if (!RS) {
      ...
      if (!OffsetReg && HasLEA &&
        ...
        if (!OffsetReg) {
          int64_t FinalDisp = 0;
          bool KeepFinalDisp = chooseFinalDisp(NewOffset, FinalDisp);
          int64_t LEAAdjust = KeepFinalDisp ? NewOffset - FinalDisp : NewOffset;
          Register SpillableOffset =
              ManualSpillAddCost < countLEAChunks(LEAAdjust) * 3
                  ? selectSpillableNoOverlap(OffsetRC, /*Exclude0=*/ScratchReg,
                                             /*Exclude1=*/BaseReg)
                  : Register();
          if (SpillableOffset) {
            ...
            LLVM_DEBUG(dbgs() << "  Using LEA chains with ScratchReg="
                              << TRI.getName(ScratchReg) << "\n");
            copyRegister(MBB, II, DL, ScratchReg, BaseReg);
            emitChunkedLEAAdjust(ScratchReg, LEAAdjust);
  ...
  if (ScratchReg) {
    ...
    emitOffsetAdd(TempReg, OffsetReg, NewOffset, SpillOffsetReg);
    if (IllegalLEA) {
    ...
    if (const auto *Mapping = Z80::getZ80FrameAddrMapping(Opc)) {
      Opc = Mapping->PointerOpcode;
    } else if (Opc == Z80::LEA24ro || Opc == Z80::LEA16ro) {
      copyRegister(MBB, ++II, DL, MI.getOperand(0).getReg(),
                   MI.getOperand(FIOperandNum).getReg());
      MI.eraseFromParent();
      return true;
    } else {
      llvm_unreachable("unsupported frame opcode");
    }
    MI.setDesc(get(Opc));
    MI.removeOperand(FIOperandNum + 1);
    if (Is24Bit)
  ...
  // for the base reg push/adjust/pop fallback, keep the current order of
  // preference. first try an unused offset temp, then a no spill
  // scavenged temp, and only then choose between LEA chunks and a controlled
  // manual spill based on the predicted local code size
  if (HasLEA) {
    if (!isInt<8>(NewOffset) && RS) {
      ...
      if (UnusedOffset) {
        ...
        if (ScavengedReg) {
          ...
          KeepFinalDisp = chooseFinalDisp(NewOffset, FinalDisp);
          int64_t LEAAdjust = KeepFinalDisp ? NewOffset - FinalDisp : NewOffset;
          Register SpillableOffset =
              ManualSpillAddCost < countLEAChunks(LEAAdjust) * 3
                  ? selectSpillableNoOverlap(OffsetRC, /*Exclude0=*/BaseReg,
                                             /*Exclude1=*/Register())
                  : Register();
          if (SpillableOffset) {
      ...
      if (isInt<8>(NewOffset))
        ...
        emitOffsetAdd(BaseReg, OffsetReg, NewOffset);
      } else {
        Register ScavengedReg = RS->scavengeRegisterBackwards(
            *OffsetRC, II, /*RestoreAfter=*/false, SPAdj, /*AllowSpill=*/false);
        if (ScavengedReg) {
...
bool Z80InstrInfo::isReMaterializableImpl(const MachineInstr &MI) const {
  switch (MI.getOpcode()) {
  ...
  // v18 folded the generic remat logic into this hook. Preserve the pre-v18
  // behavior by deferring to the generic target-independent checks.
...
void Z80InstrInfo::reMaterialize(MachineBasicBlock &MBB,
  ...
  if (!Orig.modifiesRegister(Z80::F, &RI) ||
      MBB.computeRegisterLiveness(&RI, Z80::F, I) ==
          MachineBasicBlock::LQR_Dead)
    return TargetInstrInfo::reMaterialize(MBB, I, DstReg, SubIdx, Orig);
  ...
  NewMI.substituteRegister(OrigReg.getReg(), DstReg, SubIdx, RI);
}
...
bool Z80InstrInfo::expandPostRAPseudo(MachineInstr &MI) const {
  ...
  // --- Z80 GlobalISel 8-bit Pseudo Expansion ---
  ...
  case Z80::LD8r0:
    if (MI.getOperand(0).getReg() == Z80::A) {
      ...
      MI.findRegisterDefOperand(Z80::F, /*TRI=*/nullptr, /*isDead=*/false,
                                /*Overlap=*/false)
          ->ChangeToImmediate(0);
    }
    break;
  ...
  case Z80::Cmp16a0:
  case Z80::Cmp24a0: {
    // Zero-comparison: compare $src against 0 and set S/Z accordingly.
    //
    // The instruction itself is a post-RA pseudo; expand to a sequence that
    // preserves the source register value. Only S/Z are relied upon for
    // equality and signed comparisons against zero.
    //
    ...
    // src is HL/UHL: use an arbitrary helper register (BC/UBC) without
    // clobbering it. Its value doesn't matter; S/Z depend only on the final
    // result, which is preserved as src by doing (src + helper) - helper.
    BuildMI(MBB, MI, DL, get(AddOpc), HLReg)
        .addReg(HLReg)
        .addReg(HelperReg, RegState::Undef);
    expandPostRAPseudo(*BuildMI(MBB, MI, DL, get(Z80::RCF)));
...
bool Z80InstrInfo::optimizeCompareInstr(MachineInstr &CmpInstr, Register SrcReg,
  ...
  if (CpOpc) {
    int DeadDef =
        CmpInstr.findRegisterDefOperandIdx(Z80::A, nullptr, /*isDead*/ true);
    if (DeadDef == -1)
      return false;
...
MachineInstr *Z80InstrInfo::optimizeLoadInstr(MachineInstr &MI,
  ...
  bool SawStore = false;
  if (!DefMI || !DefMI->isSafeToMove(SawStore))
    return nullptr;
...
void Z80InstrInfo::updateOperandRegConstraints(MachineFunction &MF,
  ...
  for (unsigned Idx : llvm::seq(0u, NewMI.getNumOperands())) {
    ...
    auto *NewRC =
        MRI.constrainRegClass(Reg, getRegClass(NewMI.getDesc(), Idx));
    if (!NewRC) {
      LLVM_DEBUG(
