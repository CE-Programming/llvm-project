...
Legalizer::legalizeMachineFunction(MachineFunction &MF, const LegalizerInfo &LI,
  ...
  do {
    ...
    while (!InstList.empty()) {
      ...
      // COPY can be placed on the worklist to allow dead-copy cleanup when its
      // users change, but it is not a pre-isel generic opcode and must not be
      // sent through LegalizerInfo action dispatch.
