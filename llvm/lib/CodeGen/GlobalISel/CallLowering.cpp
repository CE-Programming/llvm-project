...
bool CallLowering::lowerCall(MachineIRBuilder &MIRBuilder, const CallBase &CB,
  ...
  Info.OrigRet = ArgInfo{ResRegs, CB, getAttributesForReturn(CB)};
...
static void buildCopyToRegs(MachineIRBuilder &B, ArrayRef<Register> DstRegs,
  ...
  if (!SrcTy.isVector() && !PartTy.isVector()) {
    ...
    for (Register DstReg : DstRegs) {
      B.buildExtract(DstReg, SrcReg, Index);
      Index += PartSize;
    }
    return;
  }
  ...
  for (unsigned i = 0, j = 0; i != NumArgs; ++i, ++j) {
    ...
    if (VA.getLocInfo() == CCValAssign::Indirect) {
      ...
      if (Handler.isIncomingArgumentHandler())
        ...
        Handler.assignValueToAddress(Args[i].OrigRegs[0], ArgReg, ValTy, MPO,
                                     IndirectVA);
        IndirectOffset += Size;
      }
    ...
    if (NumParts != 1 || ValTy != OrigTy) {
      ...
      for (unsigned Part = 0; Part < NumParts; ++Part) {
        ...
        Args[i].Regs[Part] = MRI.createGenericVirtualRegister(LLT(VA.getValVT()));
      }
    }
    ...
    if (Handler.isIncomingArgumentHandler()) {
      ...
      if (VA.getLocInfo() == CCValAssign::Indirect) {
        ...
        Handler.assignValueToAddress(Args[i].OrigRegs[0], AddrReg, ValTy, MPO,
                                     IndirectVA);
        IndirectOffset += Size;
      }
    }
